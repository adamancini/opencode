...
const main = Effect.gen(function* () {
  ...
  for (const signal of ["SIGINT", "SIGTERM"] as const) {
    process.on(signal, () => {
      void killAllServers().finally(() => app.exit(0))
    })
  }
  ...
  registerIpcHandlers({
    ...
    getWslServersState: () => wslServers?.getState() ?? { runtime: null, installed: [], online: [], distroProbes: {}, opencodeChecks: {}, pendingRestart: false, servers: [], job: null },
    ...
    wslServersAddServer: (distro) => {
      if (!wslServers) throw new Error("WSL servers not initialized")
      return wslServers.addServer(distro)
    },
    ...
    runUpdater: async (alertOnFail) => checkForUpdates(alertOnFail, killAllServers),
    checkUpdate: async () => checkUpdate(),
    installUpdate: async () => installUpdate(killAllServers),
  ...
  if (mainWindow) {
    createMenu({
      ...
      checkForUpdates: () => {
        void checkForUpdates(true, killAllServers)
      },
